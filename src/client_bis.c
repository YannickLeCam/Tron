...
int main(int argc, char const *argv[])
{
    ...
    int quit = 0;
    // boucle de reception du jeu.
    // TODO notre jeu doit faire le premier affichage ici alors
    while (quit != 1)
    {
        while (affi.winner == 0) // si nous avons un gagnant notre jeu se termine
        {
            ...
            /* reception de la grille + affichage  */
            if (FD_ISSET(masock, &rfds))
            {
                CHK(recv(masock, &affi, sizeof(display_info), 0)); // a verifier
                affichage(affi);
                refresh();
            }
        }
